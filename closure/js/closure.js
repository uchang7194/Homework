...
// `warningMaker`라는 함수와 `monster`이라는 이름의 매개변수를 만든다. 
// 이 함수 내에서는 특정 `monster`를 마주쳤을 때에 기반하여 특정한 `alert` 메시지를 보여주는 익명함수를 반환한다. 
...
function warningMaker(monster) {
    var count = 0, zone = [], list = "";

    function arrayToString() {
        list = "";
        for(var i = 0, len = zone.length; i < len; i++) {
            list += zone[i][0] + " (" + zone[i][1] + ")" + "\n";
        }
        return list;
    }
...
var greenHerk = warningMaker("green Herk");
var snowYetiAlert = warningMaker("snowYeti");

// greenHerk(6, "Gwanghwa-mun");
